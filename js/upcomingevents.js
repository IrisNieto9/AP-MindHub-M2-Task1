async function getData () {
    ...
    .then((response)=>{
        ...
        const whereUpcoming = document.querySelector('section.secard')


        let cardUpcomingFiltradas = response.events.filter(card => card.date > response.currentDate)

        let renderCards = (data, where)=>{
            where.innerHTML = ''
            for (let event of data){
                where.innerHTML += `
                <div class="card d-flex justify-content-center bg-body-tertiary m-2 rounded border-danger-subtle" style="width: 20rem;">
