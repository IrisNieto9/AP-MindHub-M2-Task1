async function getData () {
    ...
    .then((response)=>{
        const wherePast = document.querySelector('section.secard')
        let cardPastFiltradas = response.events.filter(card => card.date < response.currentDate)

        let renderCards = (data, where)=>{
            where.innerHTML = '';
            for (let event of data){
                where.innerHTML += `
                <div class="card d-flex flex-wrap justify-content-center m-2 bg-body-tertiary rounded border-danger-subtle" style="width: 20rem;">
        ...
        let renderCardsSearch = (array)=>{
            ...
            array.forEach(card => {
                template.push(`<div class="card justify-content-center m-2 bg-body-tertiary rounded border-danger-subtle" style="width: 20rem;">
                <div class="image-card m-2">
            ...
            if(array.length > 0){
                ...
                return `<h2 class="fs-3">There are no coincidences</h2>`;
            }
