...
let renderCards = (data, where)=>{
    ...
    for (let event of data){
        where.innerHTML += `
        <div class="card d-flex flex-wrap justify-content-center bg-body-tertiary rounded border-danger-subtle">
...
let renderCardsSearch = (array)=>{
    ...
    array.forEach(card => {
        template.push(`<div class="card d-flex flex-wrap justify-content-center bg-body-tertiary rounded border-danger-subtle">
        <div class="image-card m-2">
            <img src="${card.image}" alt="Cinema card" class="card-img-top rounded-3">
        </div>
        <div class="detail-card text-center m-3">
            <h3>${card.name}</h3>
